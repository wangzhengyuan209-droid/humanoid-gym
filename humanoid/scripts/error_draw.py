...
def run_mujoco(policy, cfg):
    ...
    # 存储绘图数据
    plot_data = {'time': [], 'q_actual': [], 'q_target': []}
    joint_names = [
    ...
    for _ in tqdm(range(sim_steps), desc="Simulating..."):
        ...
        plot_data['q_actual'].append(q.copy())
        plot_data['q_target'].append(target_q.copy())
    ...
    actuals = np.array(plot_data['q_actual'])
    targets = np.array(plot_data['q_target'])

    fig, axs = plt.subplots(6, 2, figsize=(15, 18), sharex=True)
    fig.suptitle('Joint Tracking Performance (Policy Target vs. Actual)', fontsize=16)

    for i in range(12):
        row, col = i % 6, i // 6
        axs[row, col].plot(times, targets[:, i], label='Target', color='black', linestyle='-', linewidth=1.2, alpha=0.7)
        axs[row, col].plot(times, actuals[:, i], label='Actual', color='red', linestyle='-', linewidth=1.5, alpha=0.9)
    ...
    plt.savefig('joint_tracking.png')
    print("✅ 跟踪曲线图已保存为 'joint_tracking.png'")
    plt.show()
...
if __name__ == '__main__':
    ...
    class Sim2simCfg(UEICfg):
        ...
        class robot_config:
            ...
            kds = np.array([15, 15, 15, 15, 1, 1, 15, 15, 15, 15, 1, 1], dtype=np.double)
